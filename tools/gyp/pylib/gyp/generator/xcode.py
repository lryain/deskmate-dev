...
def GenerateOutput(target_list, target_dicts, data, params):
  ...
  for build_file, build_file_dict in data.iteritems():
    ...
    xcodeproj_path = build_file_root + options.suffix + '.xcodeproj'
    if options.generator_output:
      # if project path starts with ../ remove it
      # this fixes a bug where projects are not generated in the generator_output path
      fixed_xcodeproj_path = re.sub(r"^(\/?)(\.\./)*(.*)$", r"\1\3", xcodeproj_path)
      xcodeproj_path = os.path.normpath(os.path.join(options.generator_output, fixed_xcodeproj_path))
