...
def GenerateOutput(target_list, target_dicts, data, params):
  ...
  def CalculateMakefilePath(build_file, base_name):
    ...
    if options.generator_output:
        fixed_output_file = re.sub(r"^(\/?)(\.\./)*(.*)$", r"\1\3", output_file)
        output_file = os.path.normpath(os.path.join(options.generator_output, base_name))
  ...
  if options.generator_output:
      fixed_makefile_path = re.sub(r"^(\/?)(\.\./)*(.*)$", r"\1\3", makefile_path)
      makefile_path = os.path.normpath(os.path.join(options.generator_output, fixed_makefile_path))
