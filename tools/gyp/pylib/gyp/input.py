...
class DependencyGraphNode(object):
  ...
  def FlattenToList(self):
    ...
    in_degree_zeros = OrderedSet(self.dependents[:])
    ...
    while in_degree_zeros:
      ...
      for node_dependent in sorted(node.dependents, key=lambda node: node.ref):
        is_in_degree_zero = True
        # TODO: We want to check through the
        # node_dependent.dependencies list but if it's long and we
        # always start at the beginning, then we get O(n^2) behaviour.
        for node_dependent_dependency in sorted(node_dependent.dependencies, key=lambda node: node.ref):
