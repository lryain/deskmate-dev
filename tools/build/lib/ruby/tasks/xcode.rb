...
class XcodeProject
  ...
  def scheme
    @scheme ||= File.basename(self.project_root)
  end
  ...
  def project_name
    @project_name ||= File.basename(self.project_root)
  end
...
#    %{SYMROOT="#{self.configuration_build_dir}" DSTROOT="#{self.configuration_build_dir}" OBJROOT="#{self.configuration_build_dir}" }
  ...
  def build_args(config, profile_path=nil, platform='iphoneos')
    ...
    %{-workspace #{self.workspace} -scheme #{self.scheme} -configuration #{config} #{xcconfig_option} #{signing_args} #{self.build_dir_settings(config, platform)}}
  end
  ...
  def build(config, profile_path=nil, platform='iphoneos', command='build')
    ...
    # build!
    cmd = %{#{self.buildtool} #{self.build_args(config, profile_path)} #{command} }
  ...
  def test(opts={})
    ...
    test_option_keys = [:only, :test_sdk, :parallelize, :logic_test_bucket_size, :destination]
    test_options = {}
    ...
    command = %Q{#{XCTOOL} -workspace #{self.workspace} -scheme #{self.scheme} -sdk iphonesimulator7.1 #{options_string} test -freshSimulator -resetSimulator #{test_options_string} #{output_command} #{self.build_dir_settings('Release')}}
    system command
  end
end
