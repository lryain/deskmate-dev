...
class Session:
    def __init__(self):
        self._url = 'https://session-certs.token.global.anki-services.com/'
...
def get_cert(session, serial=None):
    serial = get_serial(serial)
    print("\nDownloading Vector certificate from Anki...", end="")
    sys.stdout.flush()
    r = requests.get('{}/vic/{}'.format(session.url, serial))
...
def main(api, session):
    ...
    cert, serial = get_cert(session, args.serial)
