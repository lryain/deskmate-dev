...
class json_int((1).__class__):  # Have to specify base this way to satisfy 2to3
  ...
  def __new__(cls, *args, **kwargs):
    if 'number_format' in kwargs:
      ...
      NUMBER_FORMAT_DECIMAL, NUMBER_FORMAT_HEX, NUMBER_FORMAT_OCTAL, NUMBER_FORMAT_LEGACYOCTAL, NUMBER_FORMAT_BINARY):
...
class JSON(object):
  ...
  def set_hook(self, hookname, function):
    ...
    if hookname in self.all_hook_names:
      att = hookname + '_hook'
      if function != None and not isinstance(function, collections.Callable):
    ...
    while i < imax:
      ...
      if cord < 256 and asciiencodable[cord] and isinstance(encunicode, bool) \
        ...
        if always_escape and c in always_escape:
          ...
          doesc = True
        elif isinstance(encunicode, collections.Callable):
          doesc = encunicode(c)
        else:
          doesc = encunicode
  ...
  def _classify_for_encoding(self, obj):
    ...
    if obj is None:
      ...
      if isinstance(obj, dict):
        c = 'dict'
      elif isinstance(obj, tuple) and hasattr(obj, '_asdict') and isinstance(obj._asdict, collections.Callable):
        # Have a named tuple
        enc_nt = self.options.encode_namedtuple_as_object
        if enc_nt and (enc_nt is True or (isinstance(enc_nt, collections.Callable) and enc_nt(obj))):
          c = 'namedtuple'
        else:
          c = 'sequence'
  ...
  def encode(self, obj, encoding=None):
    ...
    if self.options.escape_unicode and isinstance(self.options.escape_unicode, collections.Callable):
    ...
    if it is not None:
            ...
            if not helpers.isstringtype(obj2):
              if helpers.isnumbertype(obj2):
                ...
                raise JSONEncodeError(
                  'object properties (dictionary keys) can only be strings or numbers in ECMAScript', obj2)
      ...
      if isdict:
        ...
        if not srt or srt in (SORT_NONE, SORT_PRESERVE):
          srt = None
        elif isinstance(srt, collections.Callable):
  ...
  def encode_equivalent(self, obj, state):
    """This method is used to encode user-defined class objects.
        ...
        """
    if hasattr(obj, 'json_equivalent') \
        and isinstance(getattr(obj, 'json_equivalent'), collections.Callable):
