function deskmatehelp() {
cat <<EOF
Invoke ". project/deskmate/envsetup.sh" from your shell to add the following functions to your environment:
...
# project/deskmate/build-deskmate.sh
function gettop
{
    local TOPFILE=project/deskmate/build-deskmate.sh
...
{
    local cmake="`/usr/bin/which cmake`"
    if [ -z ${cmake} ]; then
        local toplevel=$(gettop)
        ${toplevel}/tools/build/tools/lryabuild/cmake.py --install-cmake
        cmake="`${toplevel}/tools/build/tools/lryabuild/cmake.py --find-cmake`"
        cmake=$(dirname ${cmake})
        pathupdate ${cmake}
    fi
}
