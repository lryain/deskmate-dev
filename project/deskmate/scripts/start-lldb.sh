#!/usr/bin/env bash
#
# project/deskmate/scripts/start-lldb.sh
#
# Helper script to enable use of lldb-server on deskmate robot
#
set -u
...
# affected by deployments into /lrya.
#
DEPLOY_FROM="${TOPLEVEL}/3rd/lldb-server/mateos/bin/lldb-server"
DEPLOY_TO="/usr/bin/lldb-server"

robot_sh "test -f ${DEPLOY_TO}"
if [ $? -ne 0 ]; then
  echo "Deploy lldb-server to ${LRYA_ROBOT_HOST}"
  robot_cp ${DEPLOY_FROM} ${DEPLOY_TO}
  if [ $? -ne 0 ]; then
    echo "Failed to deploy lldb-server to ${LRYA_ROBOT_HOST}"
    exit 1
  fi
fi
...
if [ $? -ne 0 ]; then
  echo "Failed to open firewall on ${LRYA_ROBOT_HOST}"
  exit 1
fi
...
# but it is not able to find libc++.so.6 installed into /lrya/lib.
# We start the server process with LD_LIBRARY_PATH set to search both /usr/lib and /lrya/lib.
#

PORT="55001"

echo "Starting lldb-server on ${LRYA_ROBOT_HOST}:${PORT}"
echo "LD_LIBRARY_PATH=/usr/lib:/lrya/lib nohup logwrapper -- ${DEPLOY_TO} platform --server --listen *:${PORT} </dev/null >/dev/null 2>&1 &"

# robot_sh "LD_LIBRARY_PATH=/usr/lib:/lrya/lib nohup logwrapper -- ${DEPLOY_TO} platform --server --listen *:${PORT} </dev/null >/dev/null 2>&1 &"
# if [ $? -ne 0 ]; then
#   echo "${DEPLOY_TO} failed to start"
#   exit 1
# fi

echo "${DEPLOY_TO} now running on ${LRYA_ROBOT_HOST}:${PORT}"
